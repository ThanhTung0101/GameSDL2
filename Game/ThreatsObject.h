...
const int UNDER_LIMIT_THREAT = 200; //giới hạn màn hình dưới của địch
...
public:
	...
	void HandleMove(const int & x_bordr, const int& y_border); //Giới hạn di chuyển của địch
	...
	void Reset(const int& xboder); // xóa địch
	void ResetAmo(AmoObject* p_amo); // xóa đạn
